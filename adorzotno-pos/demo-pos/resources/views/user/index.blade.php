...
<div class="list-root">
    ...
    <div class="list-head">
        <div>
            <h2><i class="bi bi-person-badge" style="color:#4f46e5;font-size:1.3rem"></i> Users</h2>
            <p>Manage system users and their roles</p>
        </div>
        <a href="{{ route('user.create') }}" class="btn-new">
            <i class="bi bi-plus-lg" style="font-size:15px"></i> Add User
        </a>
    </div>
    ...
    <div class="filter-bar">
        <div class="filter-row">
            <div class="filter-search">
                ...
                <input type="text" id="fS" placeholder="Search name or email…" autocomplete="off">
    ...
    <div class="table-card">
        <div class="table-responsive">
            <table id="userTable" class="table table-striped mb-0"></table>
        </div>
    </div>
...
$(document).ready(function () {
    ...
    var tbl = $('#userTable').DataTable({
        ...
        columns: [
            ...
            { title:'Phone',        data:'phone_display', name:'phone',        className:'text-center', orderable:false },
            { title:'Role Summary', data:'role_summary',  name:'role_summary', className:'text-center', orderable:false, searchable:false },
            { title:'Staff',        data:'staff_badge',   name:'staff_badge',  className:'text-center', orderable:false, searchable:false },
            { title:'Status',       data:'status_badge',  name:'status',       className:'text-center', orderable:false, searchable:false },
            { title:'Action', className:'text-center', data:function(d){ return '<a title="edit" class="btn btn-warning btn-xs me-2" data-panel-id="'+d.id+'" onclick="editUser(this)"><i class="fa fa-edit"></i></a><a title="delete" class="btn btn-danger btn-xs" data-panel-id="'+d.id+'" onclick="deleteUser(this)"><i class="fa fa-trash"></i></a>'; }, orderable:false, searchable:false }
        ]
    });
...
function editUser(x) { window.location.href = '{{ route("user.edit", ":id") }}'.replace(':id', $(x).data('panel-id')); }
function deleteUser(x) {
    if (!confirm("Delete This User?")) return false;
    $.ajax({ type:'POST', url:"{!! route('user.delete') !!}", data:{ _token:"{{ csrf_token() }}", id:$(x).data('panel-id') },
        success:function(){ toastr.success('User deleted successfully!'); $('#userTable').DataTable().ajax.reload(null,false); },
        error:function(xhr){ toastr.error(xhr.responseJSON?.message||'Unable to delete user.'); }
    });
}
</script>
@endsection
