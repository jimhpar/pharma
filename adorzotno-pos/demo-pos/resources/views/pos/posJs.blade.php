    ...
    <div class="" data-search="${searchAttr}" onclick="openProductSelection(${product.id}, ${product.action_sku_id ?? 'null'}, ${product.is_variant_product ? 'true' : 'false'}, ${product.is_medicine ? 'true' : 'false'})">
        <div class="product-card">
            ...
            <div class="product-card-body">
                ...
                ${pharmaParts.length ? `<div class="pos-pharma-sub">${pharmaParts.join(' · ')}</div>` : ''}
                <hr class="pc-divider">
...
function openProductSelection(productId, actionSkuId, isVariantProduct, isMedicineProduct = false) {
    if (actionSkuId && !isMedicineProduct) {
        // Direct-add: check for boxes first, show picker if any exist
        fetchBoxes(actionSkuId, function (boxes) {
            if (!boxes.length) {
                addToCart(actionSkuId);
            } else {
                _boxPickerCallback = function (boxId) { addToCart(actionSkuId, false, 'unit', boxId); };
                openBoxPickerModal(boxes);
            }
        });
        return;
    }
...
function fetchBoxes(skuId, callback) {
...
function loadVariantBoxes() {
    ...
    $section.html('<div class="vbs-loading"><span class="spinner-border spinner-border-sm"></span> Loading box stock...</div>');

    fetchBoxes(sku.id, function (boxes) {
        if (!boxes.length) { $section.html(''); return; }
        ...
        $section.html(`
            <div class="vbs-block">
                <div class="vbs-label">
                    <i class="bi bi-box-seam"></i> Sell From Box
                    <small>— lowest stock pre-selected</small>
                </div>
...
function openBoxPickerModal(boxes) {
    selectedBoxId = boxes.length ? boxes[0].id : null;

    const $body = $('#boxPickerBody');
    $body.html(
        boxes.length
            ? `<div class="bp-list">${renderBoxOptions(boxes)}</div>`
            : '<p class="text-muted text-center py-3">No box data found. Cart will use auto allocation.</p>'
    );
    wireBoxRadios($body);
...
$(document).ready(function () {
    ...
    $('#loadMoreProductsBtn').on('click', function () {
        if (isProductLoading || currentProductPage >= lastProductPage) {
            return;
        }
    ...
    $('#variantAddToCartBtn').on('click', function () {
        ...
        addToCart(selectedSku.id, true, selectedSku.is_medicine ? selectedSaleUnit : 'unit', selectedBoxId);
    });
    ...
    $(document).on('click', '#boxPickerConfirmBtn', function () {
        ...
        setTimeout(function () { cleanupModalBackdrop(); if (cb) cb(selectedBoxId); }, 250);
    });
...
// Add to cart
function addToCart(skuId, closeModal = false, saleUnit = 'unit', boxId = null) {
    let quantity = 1;
    $.ajax({
        type: 'POST',
...
function updateCartTotal() {
    ...
    if (order_discount > baseSubtotal) {
        alert("Discount cannot be greater than subtotal!");
        order_discount = 0;
        $('#order_discount').val(order_discount);
    }

    const taxableAmount = Math.max(0, baseSubtotal - order_discount);
...
$(document).ready(function () {
    ...
    $(document).on('input', '#delivery_charge, #order_discount, #vat_percent', function () {
        updateCartTotal();
    });
