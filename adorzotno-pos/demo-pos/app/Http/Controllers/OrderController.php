...
class OrderController extends Controller
{
    public function show(): View
    {
        ...
        $cashiers = User::query()
            ...
            ->when($this->currentBranchId(), fn ($query, $branchId) => $query->where('sales_orders.branch_id', $branchId))
            ->distinct()
            ->orderBy('users.name')
            ->get();
    ...
    public function list(Request $request)
    {
        $orders = SalesOrder::query()
            ...
            ->when($request->filled('branch_id'), fn ($query) => $query->where('sales_orders.branch_id', (int) $request->input('branch_id')))
