...
class ReportController extends Controller
{
    ...
    public function productSalesShow()
    {
    ...
    public function productSalesList(Request $request)
    {
        ...
        return DataTables()->of($items)
            ...
            ->addColumn('sales_channel', fn (SalesOrderItem $i) => $i->salesOrder?->sales_channel ?? 'N/A')
    ...
    private function productSalesQuery(Request $request)
    {
        ...
        return SalesOrderItem::query()
            ...
            ->whereHas('salesOrder', function ($q) use ($branchId, $request) {
                ...
                if ($request->filled('salesChannel')) {
                    $q->where('sales_channel', $request->salesChannel);
                }
            })
    ...
    public function branchStockShow()
    {
        ...
        $totals = StockBalance::query()
            ->select([
                ...
                DB::raw('SUM(available_quantity * (SELECT cost_price FROM product_skus WHERE id = stock_balances.sku_id)) as total_value'),
            ])
