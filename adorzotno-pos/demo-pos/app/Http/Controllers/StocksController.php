...
class StocksController extends Controller
{
    ...
    public function list(Request $request)
    {
        ...
        return DataTables()->of($stocks)
            ...
            ->addColumn('cost_price', fn ($row) => number_format((float) ($row->cost_price ?? 0), 2))
            ->addColumn('retail_price', fn ($row) => number_format((float) ($row->retail_price ?? 0), 2))
            ->addColumn('stock_value', function ($row) {
                $value = (int) $row->available_stock * (float) ($row->cost_price ?? 0);
    ...
    public function summary(Request $request)
    {
        ...
        $totalValue = $rows->sum(fn ($r) => (int) $r->available_stock * (float) ($r->cost_price ?? 0));
    ...
    public function excel(Request $request)
    {
        $rows = $this->stockReportQuery($request)->get()->map(fn ($row) => [
            ...
            number_format((int) $row->available_stock * (float) ($row->cost_price ?? 0), 2),
