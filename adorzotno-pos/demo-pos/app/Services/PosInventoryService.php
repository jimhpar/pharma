...
class PosInventoryService
{
    ...
    public function deductSkuStockForSale(
        ...
        $this->deductFromBoxes($skuId, $warehouseId, $quantity, $preferredBoxId);

        return $allocations;
    }

    private function deductFromBoxes(int $skuId, int $warehouseId, int $quantity, ?int $preferredBoxId = null): void
    {
        if ($quantity <= 0) {
            return;
        }
