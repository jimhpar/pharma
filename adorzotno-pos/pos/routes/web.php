...
use App\Http\Controllers\DiscountManagerController;
...
Route::get('/sso-login', function (\Illuminate\Http\Request $request) {
    ...
    if ($token) {
        ...
        if (count($parts) === 2) {
            [$id, $plainToken] = $parts;
            $tokenRecord = \Illuminate\Support\Facades\DB::table('personal_access_tokens')->find($id);
...
Route::get('/public/{path}', function ($path) {
    ...
    if (file_exists($fullPath) && !is_dir($fullPath)) {
        $headers = [];
        if (str_ends_with($fullPath, '.css')) $headers['Content-Type'] = 'text/css';
        if (str_ends_with($fullPath, '.js')) $headers['Content-Type'] = 'application/javascript';
...
Route::middleware(['auth', 'initialize.branch'])->group(function () {
    ...
    Route::post('/list', [FaqController::class, 'list'])->name('list');
    ...
    Route::get('/show', [CouponController::class, 'show'])->name('show');
    Route::post('/list', [CouponController::class, 'list'])->name('list');
    ...
    Route::get('brand-search', [ProductController::class, 'brandSearch'])->name('brandSearch');
    Route::get('generate-sku-code', [ProductController::class, 'generateSkuCode'])->name('generateSkuCode');
    ...
    // Discount Manager
    Route::group(['prefix' => 'discount-manager', 'as' => 'discountManager.', 'middleware' => 'require.permission:discount.manage'], function () {
        Route::get('/',        [DiscountManagerController::class, 'show'])->name('show');
        Route::get('/products',[DiscountManagerController::class, 'products'])->name('products');
        Route::post('/apply',  [DiscountManagerController::class, 'apply'])->name('apply');
        Route::post('/remove', [DiscountManagerController::class, 'remove'])->name('remove');
    });

    // Purchase Requisition
    Route::group(['prefix' => 'purchase-requisition', 'as' => 'purchaseRequisition.', 'middleware' => 'require.permission:purchases.requisition_manage,purchases.requisition_approve'], function () {
        Route::get('/show',        [PurchaseRequisitionController::class, 'show'])->name('show');
        Route::post('/list',       [PurchaseRequisitionController::class, 'list'])->name('list');
        Route::get('/create',      [PurchaseRequisitionController::class, 'create'])->name('create')->middleware('require.permission:purchases.requisition_manage');
        Route::post('/store',      [PurchaseRequisitionController::class, 'store'])->name('store')->middleware('require.permission:purchases.requisition_manage');
        Route::get('/edit/{id}',   [PurchaseRequisitionController::class, 'edit'])->name('edit')->middleware('require.permission:purchases.requisition_manage');
        Route::post('/update/{id}',[PurchaseRequisitionController::class, 'update'])->name('update')->middleware('require.permission:purchases.requisition_manage');
        Route::post('/submit/{id}',[PurchaseRequisitionController::class, 'submit'])->name('submit');
        Route::post('/approve/{id}',[PurchaseRequisitionController::class, 'approve'])->name('approve')->middleware('require.permission:purchases.requisition_approve');
        Route::post('/reject/{id}', [PurchaseRequisitionController::class, 'reject'])->name('reject')->middleware('require.permission:purchases.requisition_approve');
        Route::post('/convert/{id}',[PurchaseRequisitionController::class, 'convertToPo'])->name('convertToPo')->middleware('require.permission:purchases.requisition_approve');
        Route::post('/delete',     [PurchaseRequisitionController::class, 'delete'])->name('delete');
        Route::get('/sku-search',  [PurchaseRequisitionController::class, 'skuSearch'])->name('skuSearch');
        Route::get('/pdf/{id}',    [PurchaseRequisitionController::class, 'downloadPdf'])->name('pdf');
    });

    // Available Stock Management
    Route::group(['prefix' => 'available-stock', 'as' => 'availableStock.', 'middleware' => 'require.permission:reports.inventory.view'], function () {
        Route::get('/show', [AvailableStockController::class, 'show'])->name('show');
        Route::post('/list', [AvailableStockController::class, 'list'])->name('list');
        Route::get('/product-details/{id}', [AvailableStockController::class, 'productDetails'])->name('productDetails');
        Route::post('/store-stock', [AvailableStockController::class, 'storeStock'])->name('storeStock');
        Route::post('/update-stock/{id}', [AvailableStockController::class, 'updateStock'])->name('updateStock');
        Route::post('/delete-stock/{id}', [AvailableStockController::class, 'deleteStock'])->name('deleteStock');
    });
    ...
    Route::group(['prefix' => 'pos', 'as' => 'pos.', 'middleware' => 'require.permission:pos.access'], function () {
        ...
        Route::get('/alternativeMedicines', [PosController::class, 'alternativeMedicines'])->name('alternativeMedicines');
        Route::get('/sku/{skuId}/batches', [BoxStockController::class, 'batchesForSku'])->name('sku.batches');
