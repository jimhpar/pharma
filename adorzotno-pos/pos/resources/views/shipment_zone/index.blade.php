...
<div class="list-root">
    ...
    <div class="list-head">
        <div>
            <h2><i class="bi bi-truck" style="color:#4f46e5;font-size:1.3rem"></i> Shipment Zones</h2>
            <p>Manage delivery zones and charges</p>
        </div>
        <a href="{{ route('shipmentZone.create') }}" class="btn-new">
            <i class="bi bi-plus-lg" style="font-size:15px"></i> Add Zone
        </a>
    </div>
    ...
    <div class="filter-bar">
        <div class="filter-row">
            <div class="filter-search">
                ...
                <input type="text" id="fS" placeholder="Search zone name…" autocomplete="off">
    ...
    <div class="table-card">
        <div class="table-responsive">
            <table id="shipmentZoneTable" class="table table-striped mb-0"></table>
        </div>
    </div>
...
$(document).ready(function () {
    var _dt;
    var tbl = $('#shipmentZoneTable').DataTable({
        processing: true, serverSide: true,
        ajax: {
            url:"{{ route('shipmentZone.list') }}", type:"POST",
            data:function(d){ d._token="{{ csrf_token() }}"; d.filter_status=$('#fSt').val(); }
        },
        columns: [
            { title:'Name',   data:'name',   name:'name',   className:"text-center" },
            { title:'Charge', data:'charge', name:'charge', className:"text-center" },
            { title:'Status', data:'status', name:'status', className:"text-center", searchable:false },
            { title:'Action', className:"text-center", data:function(d){ return '<a title="edit" class="btn btn-warning btn-xs me-2" data-panel-id="'+d.id+'" onclick="editShipmentZone(this)"><i class="fa fa-edit"></i></a><a title="delete" class="btn btn-danger btn-xs" data-panel-id="'+d.id+'" onclick="deleteShipmentZone(this)"><i class="fa fa-trash"></i></a>'; }, orderable:false, searchable:false }
        ]
    });
...
function editShipmentZone(x) { window.location.href = '{{ route("shipmentZone.edit", ":id") }}'.replace(':id', $(x).data('panel-id')); }
function deleteShipmentZone(x) {
    if (!confirm("Delete This Shipment Zone?")) return false;
    $.ajax({ type:'POST', url:"{!! route('shipmentZone.delete') !!}", data:{_token:"{{ csrf_token() }}",id:$(x).data('panel-id')},
        success:function(){ toastr.success('Shipment Zone Deleted Successfully!'); $('#shipmentZoneTable').DataTable().clear().draw(); }
    });
}
</script>
@endsection
