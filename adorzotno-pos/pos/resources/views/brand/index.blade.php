...
<div class="list-root">
    ...
    <div class="list-head">
        <div>
            <h2><i class="bi bi-award" style="color:#4f46e5;font-size:1.3rem"></i> Brands</h2>
            <p>Manage product brands</p>
        </div>
        <a href="{{ route('brand.create') }}" class="btn-new">
            <i class="bi bi-plus-lg" style="font-size:15px"></i> Add Brand
        </a>
    </div>
    ...
    <div class="filter-bar">
        <div class="filter-row">
            <div class="filter-search">
                ...
                <input type="text" id="fS" placeholder="Search brand name or slug…" autocomplete="off">
    ...
    <div class="table-card">
        <div class="table-responsive">
            <table id="brandTable" class="table table-striped mb-0"></table>
        </div>
    </div>
...
$(document).ready(function () {
    var _dt;
    var tbl = $('#brandTable').DataTable({
        processing: true, serverSide: true,
        ajax: {
            url: "{{ route('brand.list') }}", type: "POST",
        ...
        columns: [
            ...
            { title:'Brand Name', data:'name',       name:'name',       className:"text-center" },
            { title:'Slug',       data:'slug',       name:'slug',       className:"text-center" },
            { title:'Logo',       data:'logo',       name:'logo',       className:"text-center", orderable:false, searchable:false },
            { title:'Status',     data:'status',     name:'status',     className:"text-center", searchable:false },
            { title:'Action', className:"text-center", data:function(d){ return '<a title="edit" class="btn btn-warning btn-xs me-2" data-panel-id="'+d.id+'" onclick="editBrand(this)"><i class="fa fa-edit"></i></a><a title="delete" class="btn btn-danger btn-xs" data-panel-id="'+d.id+'" onclick="deleteBrand(this)"><i class="fa fa-trash"></i></a>'; }, orderable:false, searchable:false }
        ]
    });
...
function editBrand(x) { window.location.href = '{{ route("brand.edit", ":id") }}'.replace(':id', $(x).data('panel-id')); }
function deleteBrand(x) {
    if (!confirm("Delete This Brand?")) return false;
    $.ajax({ type:'POST', url:"{!! route('brand.delete') !!}", data:{_token:"{{ csrf_token() }}",id:$(x).data('panel-id')},
        success:function(){ toastr.success('Brand Deleted Successfully!'); $('#brandTable').DataTable().clear().draw(); }
    });
}
</script>
@endsection
