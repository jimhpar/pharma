...
<style>
    ...
    .product-card.out-of-stock .product-card-stock {
        color: #dc2626;
        font-weight: 700;
    }
    .product-thumb {
    ...
    html[data-bs-theme="dark"] .product-card.out-of-stock {
        border-color: #ef4444;
        border-width: 1px;
    }
    html[data-bs-theme="dark"] .product-card.out-of-stock:hover {
        border-color: #dc2626;
    ...
    .ci-disc-row .ci-discount-type {
        width: 3.25rem;
        min-height: 1.62rem;
        border: none;
        border-right: 1px solid #f3bf8a;
        background: transparent;
        color: #9a3412;
        font-size: 0.72rem;
        font-weight: 800;
    ...
    .bp-price { font-weight: 800; font-size: .88rem; color: #0f766e; }
                                    ...
                                    <table class="cart-table-view">
                                        ...
                                        <tbody>
                                            ...
                                            @php
                                                ...
                                                $displayUnitDiscount = (int) $cartItem->quantity > 0 ? $lineDiscount / (int) $cartItem->quantity : 0;
                                                $lineTotal = max(0, ((float) $cartItem->price * (int) $cartItem->quantity) - $lineDiscount);
                                                $discountType = $cartItem->attributes->discount_type ?? 'amount';
                                                $discountType = $discountType === 'fixed' ? 'amount' : $discountType;
                                                $discountInputValue = (float) ($cartItem->attributes->discount_value ?? $lineDiscount);
                                                ...
                                                $batchNo = $cartItem->attributes->batch_no ?? '';
                                            @endphp
                                            <tr class="cart-item" data-id="{{ $cartItem->id }}">
                                                ...
                                                <td class="ct-col-product">
                                                    ...
                                                    @if (!empty($batchNo))
                                                        <span class="ci-meta-sep">|</span>
                                                        <span class="ci-sku">Batch: {{ $batchNo }}</span>
                                                    @endif
                                                </div>
                                            </td>
                                            <td class="ct-col-price" style="text-align:right">
                                                ...
                                                @endif
                                                <div class="ci-price-final">{{ Currency::format(max(0, (float)$cartItem->price - $displayUnitDiscount)) }}</div>
                                            </td>
                                            <td class="ct-col-disc" style="text-align:center">
                                                <div class="ci-disc-row">
                                                    <select class="ci-discount-type no-select2"
                                                        id="discount_type_{{ $cartItem->id }}"
                                                    ...
                                                    <input type="number" class="ci-discount-input"
                                                        ...
                                                        value="{{ number_format($discountInputValue, 2, '.', '') }}"
                                                        data-previous="{{ number_format($discountInputValue, 2, '.', '') }}"
                                                        data-discount-amount="{{ number_format($lineDiscount, 2, '.', '') }}"
                                                        min="0"
                                                        step="0.01"
                                                        onchange="updateItemDiscount('{{ $cartItem->id }}')">
                                                </div>
                    ...
                    <div class="row g-2 checkout-inline-grid mb-2">
                        <div class="col-4">
                            ...
                            <div class="input-group input-group-sm">
                                <select class="form-select no-select2" id="order_discount_type" name="order_discount_type" style="max-width:76px">
                                    <option value="amount">৳</option>
                                    <option value="percent">%</option>
                                </select>
                                <input type="number" class="form-control" id="order_discount" name="order_discount" placeholder="0.00" min="0" step="0.01">
                            </div>
                        </div>
                        <div class="col-4">
                        ...
                        <div class="filter-bar" style="border-radius:10px;padding:10px 12px">
                            ...
                            <div class="filter-row" style="gap:6px">
                                <div class="filter-search" style="max-width:none;flex:1;min-width:120px">
                                    ...
                                    <input type="text" id="skuSearch" placeholder="Search by name, generic, barcode…" autocomplete="off">
                    ...
                    <div class="products-scroll p-3">
                        <div class="row row-cols-2 row-cols-md-3 row-cols-xl-4 g-2" id="skuList">                          
                            ...
                            <div class="col"
                                ...
                                <div class="product-card{{ $product->available_stock <= 0 ? ' out-of-stock' : '' }}">
                                    ...
                                    <div class="product-card-body">
                                        ...
                                        <div class="pos-generic-name">{{ $product->generic_name }}</div>
                                        @endif
...
<div class="modal fade" id="boxPickerModal" tabindex="-1" aria-hidden="true" data-bs-backdrop="false">
    <div class="modal-dialog" style="max-width:460px">
        <div class="modal-content">
            <div class="modal-header py-2">
                <h5 class="modal-title fs-6"><i class="bi bi-layers me-2"></i>Select Batch to Sell From</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
