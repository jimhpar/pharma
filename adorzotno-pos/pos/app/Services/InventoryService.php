...
class InventoryService
{
    public function receivePurchaseOrder(PurchaseOrder $purchaseOrder, array $items): PurchaseOrder
    {
        return DB::transaction(function () use ($purchaseOrder, $items) {
            ...
            foreach ($items as $index => $receivedItem) {
                ...
                // Resolve sale_price for this batch from the receive form
                $newSalePrice = isset($receivedItem['sale_price']) && $receivedItem['sale_price'] !== ''
                    ? (float) $receivedItem['sale_price']
                    : null;
                ...
                if ($newSalePrice !== null && $newSalePrice > 0) {
                    $purchaseOrderItem->sale_price = $newSalePrice;
                }
                $purchaseOrderItem->save();

                // Create carton + box records if provided
                $this->createCartonBoxRecords(
                    $purchaseOrderItem,
                    $purchaseOrder,
                    $batch,
                    $receivedItem['cartons'] ?? []
                );
    ...
    private function createCartonBoxRecords(
        ...
        foreach ($cartons as $cartonData) {
            $boxesCount = (int) ($cartonData['boxes_per_carton'] ?? 0);
            $unitsPerBox = (int) ($cartonData['units_per_box'] ?? 0);

            // Skip if no meaningful data
            if ($boxesCount <= 0 || $unitsPerBox <= 0) {
                continue;
            }

            $rawCode   = trim((string) ($cartonData['carton_code'] ?? ''));
            $cartonCode = $rawCode !== ''
                ? $rawCode
                : InventoryCarton::generateCode($purchaseOrder->id, $item->sku_id);

            // Ensure unique carton code
            $suffix = 1;
            $baseCode = $cartonCode;
            while (InventoryCarton::where('carton_code', $cartonCode)->exists()) {
                $cartonCode = $baseCode . '-' . $suffix++;
            }

            $totalUnits = $boxesCount * $unitsPerBox;

            $cartonPrice   = isset($cartonData['unit_cost']) && $cartonData['unit_cost'] !== ''
                ? (float) $cartonData['unit_cost']
                : null;
            $cartonBatchNo = trim((string) ($cartonData['batch_no'] ?? ''));
            $cartonExpiry  = $cartonData['expiry_date'] ?? null;

            $carton = InventoryCarton::create([
                'purchase_order_item_id' => $item->id,
                'sku_id'                 => $item->sku_id,
                'warehouse_id'           => $purchaseOrder->warehouse_id,
                'batch_id'               => $batch->id,
                'carton_code'            => $cartonCode,
                'boxes_per_carton'       => $boxesCount,
                'units_per_box'          => $unitsPerBox,
                'total_units'            => $totalUnits,
                'available_units'        => $totalUnits,
                'unit_cost'              => $cartonPrice,
                'batch_no_label'         => $cartonBatchNo ?: null,
                'expiry_date'            => $cartonExpiry ?: null,
                'received_at'            => now(),
            ]);

            for ($b = 1; $b <= $boxesCount; $b++) {
                InventoryBox::create([
                    'carton_id'      => $carton->id,
                    'sku_id'         => $item->sku_id,
                    'box_code'       => $cartonCode . '-B' . str_pad($b, 2, '0', STR_PAD_LEFT),
                    'box_number'     => $b,
                    'units_received' => $unitsPerBox,
                    'units_available'=> $unitsPerBox,
                    'units_sold'     => 0,
                    'status'         => 'sealed',
                ]);
            }
