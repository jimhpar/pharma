            ...
            foreach ($cartItems as $cartData) {
                ...
                if ($modeConfig['deduct_stock']) {
                    $preferredBoxId = !empty($cartData->attributes->preferred_box_id)
                        ...
                        : null;
                    $preferredBatchId = !empty($cartData->attributes->preferred_batch_id)
                        ? (int) $cartData->attributes->preferred_batch_id
                        : null;
                    ...
                    foreach ($allocations as $allocation) {
                        ...
                        // Use batch sale_price if set — gives per-batch accuracy
                        $effectiveUnitPrice = isset($allocation['sale_price']) && $allocation['sale_price'] > 0
                            ? round((float) $allocation['sale_price'] / $stockMultiplier, 2)
                            : $unitPrice;
                        ...
                        $salesOrder->items()->create([
                            ...
                            'tax_amount'      => 0,
                            'line_total'      => $this->calculateLineTotal($effectiveUnitPrice, $allocatedQuantity, $allocatedDiscount),
                            'returned_quantity' => 0,
                        ]);
                    }
                } else {
