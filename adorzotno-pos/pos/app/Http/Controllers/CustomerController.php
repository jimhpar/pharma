...
class CustomerController extends Controller
{
    ...
    {
        $this->scopeReportBranch($query, $request);

        return $query
            ->whereIn('status', SalesOrder::REPORTABLE_SALES_STATUSES)
            ->when($request->filled('salesChannel'), fn ($salesQuery) => $salesQuery->whereIn('sales_channel', SalesOrder::channelFilterValues($request->input('salesChannel'))))
    ...
    private function customerSalesReportCustomers(Request $request)
    {
        ...
        return Customer::query()
            ...
            ->whereHas('salesOrders', function ($query) use ($request) {
                $this->applySalesReportFilters($query, $request);
            })
            ->orderBy('name')
            ->get();
