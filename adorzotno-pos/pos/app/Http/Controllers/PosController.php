...
class PosController extends Controller
{
    ...
    private function buildPosProductQuery(string $query = '', array $categoryIds = [], ?int $brandId = null): Builder
    {
        ...
        return Product::query()
            ->select(['id', 'category_id', 'brand_id', 'name', 'generic_name', 'dosage_form', 'strength', 'coating_type', 'thumbnail_image', 'status'])
            ->with([
                'category:id,name',
                'brand:id,name',
                'sku:id,product_id,sku_code,barcode,retail_price,online_price,units_per_strip,medicine_unit_price,sale_price,status',
            ...
            ->when($query !== '', function ($q) use ($query) {
                ...
                // Relevance: exact name > name-starts-with > name-contains > generic match > rest
                $q->orderByRaw(
                    "CASE
                        WHEN name = ? THEN 0
    ...
    public function alternativeMedicines(Request $request): JsonResponse
    {
        $query        = trim((string) $request->get('q'));
        $genericNames = array_filter(array_map('trim', (array) $request->input('generic_names', [])));
        $excludeIds   = array_filter(array_map('intval', (array) $request->input('exclude_ids', [])));

        if ($query === '' && empty($genericNames)) {
            return response()->json(['data' => []]);
        }

        $resolvedNames = collect($genericNames);

        if ($resolvedNames->isEmpty() && $query !== '') {
            // Derive generic names from products whose name or generic_name matches the query
            $resolvedNames = Product::query()
                ->where('status', 'Active')
        ...
        $alternatives = $this->buildPosProductQuery()
            ->whereIn('generic_name', $resolvedNames->all())
            ->when(!empty($excludeIds), fn ($q) => $q->whereNotIn('id', $excludeIds))
            ->latest()
            ->limit(12)
            ->get();

        $this->appendProductCardMeta($alternatives);

        return response()->json(['data' => $this->toCompactJson($alternatives)]);
    }
