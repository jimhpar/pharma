...
export default function HomeCategoryBannerSectionTwo() {
  ...
  const sectionBanners = useMemo(() => {
    const filteredBanners = banners.filter(
      (banner) =>
        ...
        banner?.banner_type === "homepage_middle_2",
    );
  ...
  return (
    <div
      className="mb-8 grid h-[250px] grid-rows-[1.6fr_1fr] gap-3 sm:h-[340px] md:h-[420px] lg:h-[380px] lg:grid-cols-[2fr_1fr] lg:grid-rows-1 xl:h-[420px]"
    >
      <div className="order-1 h-full min-h-0 overflow-hidden rounded-xl">
        <BannerImg banner={sectionBanners.large} sizes="50vw" className="h-full" />
      </div>
      <div className="order-2 grid h-full min-h-0 grid-cols-2 gap-3 lg:grid-cols-1 lg:grid-rows-2">
