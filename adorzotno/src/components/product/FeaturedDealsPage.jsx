...
export default function FeaturedDealsPage({ initialPage = 1 }) {
  ...
  const pagination = featuredDealsResponse || null;

  const featuredDealsProducts = useMemo(
    () => (featuredDealsResponse?.data || []).map(mapApiProductToCard),
    [featuredDealsResponse?.data],
  );
  ...
  return (
    <div>
      <div className="mb-6 rounded-md bg-gradient-to-r from-primary to-primary/80 px-4 py-3 text-white">
        <div className="flex items-center gap-4">
          ...
          <div className="flex items-center gap-2 space-x-1">
            ...
            <span className="font-semibold">Featured Deals</span>
            <span className="text-sm text-gray-100">
              ({pagination?.total ?? featuredDealsProducts.length} items)
            </span>
          </div>
        </div>
      </div>
      ...
      <FilteredProductCard
        filteredProducts={featuredDealsProducts}
